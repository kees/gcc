...
mangle_builtin_type_to_buffer (tree type, char **p, char *end)
{
  ...
  switch (TREE_CODE (type))
    ...
    default:
      ...
      error ("KCFI: Unknown builtin type with %<TREE_CODE%> %d", TREE_CODE (type));
	...
	**p = 'E';
	...
	if (name)
	...
	else
	  {
	    ...
	    error ("KCFI: No struct/union/enum name found for type code %d (%qs)",
		   TREE_CODE (type), get_tree_code_name (TREE_CODE (type)));
	    if (TYPE_NAME (type))
	      {
		error ("KCFI: %<TYPE_NAME%> exists but extraction failed");
		error ("KCFI: %<TYPE_NAME%> tree code = %d", TREE_CODE (TYPE_NAME (type)));
		if (TREE_CODE (TYPE_NAME (type)) == TYPE_DECL)
		  {
		    tree decl_name = DECL_NAME (TYPE_NAME (type));
		  ...
		  {
		    error ("KCFI: Unknown %<TYPE_NAME%> tree code = %d", TREE_CODE (TYPE_NAME (type)));
		  }
	      }
	    else
	      {
